...
let startParsePrice = "value=\"";
let midParsePrice = "\" onclick=\"toggle";
let endParsePrice = "new";
let quoteParsePrice = "\'"
let priceNotAvailable = 'Not Available';

// Get price from book page
function getBookPrice(bookHTML, ISBN) {
    let priceString = bookHTML.split(`${startParsePrice}${ISBN}${midParsePrice}`);
    if (priceString.length == 1) return 'Not Available';

    priceString = priceString[1].split(endParsePrice)[1];
    ...
    if (priceString == '0.00' || priceString == '') return priceNotAvailable;
...
async function getTextbookInformation(link, program, term, dept, course, section) {
    ...
    var deptSplit = dept.split("-"); var deptAbv = deptSplit[0].trim();
    if (deptSplit.length == 1) var deptName = deptAbv;
    else var deptName = deptAbv;
    for (let book = 1; book < bookSplit.length; book++) {
        let ISBN = bookSplit[book].substring(0, 13);
        let price = getBookPrice(bookHTML, ISBN);
        ...
        let newBook = { isbn: ISBN, name: bookName, program: program, term: term, deptName: deptName, deptAbv: deptAbv, course: course, section: section, price: price, image: bookImageURL };
